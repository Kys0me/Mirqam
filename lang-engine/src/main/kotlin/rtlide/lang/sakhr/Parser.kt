...
class Parser(private val tokens: List<Token>, private val diagnostics: DiagnosticCollector) {
    ...
    private fun error(token: Token, message: String, fixes: List<QuickFix> = emptyList()): ParseError {
        ...
        diagnostics.reportError(fullMsg, token.location, token.lexeme.length.coerceAtLeast(1), fixes)
        return ParseError()
    }

    private fun error(token: Token, message: String, suggestion: String): ParseError {
        return error(token, message, listOf(QuickFix("استخدام $suggestion", suggestion)))
    }
