...
sealed class Stmt {
    data class Block(val statements: List<Stmt>, val endToken: Token? = null) : Stmt()
    ...
    data class If(val keyword: Token, val condition: Expr, val thenBranch: Stmt, val elseBranch: Stmt?) : Stmt()
    data class While(val keyword: Token, val condition: Expr, val body: Stmt) : Stmt()
    data class ForEach(
        val keyword: Token,
        val indexVar: Token?,
